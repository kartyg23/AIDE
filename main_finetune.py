...
def main(args):
    ...
    if args.eval:
        ...
        for v_id, val in enumerate(vals):
            ...
            # print("going inside")
            test_stats, ap, r_acc, f_acc, acc, r_precision, r_recall, r_f1, f_precision, f_recall, f_f1 = evaluate(data_loader_val, model, device)
            print(f"Accuracy of the network on {len(dataset_val)} test images: {test_stats['acc1']:.5f}%")
            print(f"test dataset is {val} acc: {acc}, ap: {ap}")
            print('AP: {:2.2f}, Acc: {:2.2f},'.format(ap*100., acc*100.))
            print('Acc (real): {:2.2f}, Acc (fake): {:2.2f},'.format(r_acc*100., f_acc*100.))
